...
describe('create-version-options', () => {
  test('when options created then default settings are used', () => {
    const options = VersionOptionsFactory.create(['one', 'two'], { bump: 'patch' });
    ...
    expect(options.releaseCommitMessageFormat).toMatch('chore(release): Bumping version to {{currentTag}}');
  });
