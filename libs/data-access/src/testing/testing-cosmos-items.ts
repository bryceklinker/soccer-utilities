import {
  ...
  ChangeFeedOptions,
  Container,
  FeedOptions, ItemDefinition, ItemResponse,
  Items, OperationInput, OperationResponse,
  QueryIterator, RequestOptions,
  SqlQuerySpec
} from '@azure/cosmos';
...
export class TestingCosmosItems extends Items {
  ...
  setupItem<T extends ItemDefinition>(item: T) {
    this.items.push(new TestingCosmosItem(this.container as TestingCosmosContainer, item.id, item));
  }

  query<T>(query: string | SqlQuerySpec, options?: FeedOptions): QueryIterator<T> {
    return new TestingQueryIterator(this.items.map(i => i.resource), query);
  }
  ...
  readAll<T extends ItemDefinition>(options?: FeedOptions): QueryIterator<T> {
    return new TestingQueryIterator(this.items.map(i => i.resource), '');
  }
  ...
  create<T  extends ItemDefinition = any>(body: T, options?: RequestOptions): Promise<ItemResponse<T>> {
    ...
    const item = new TestingCosmosItem(this.container as TestingCosmosContainer, created.id, created);
