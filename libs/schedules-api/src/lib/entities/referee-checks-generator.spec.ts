...
describe('RefereeChecksGenerator', () => {
  test('when game has one center referee then returns one referee check', () => {
    const game = createGame(8, ModelFactory.createReferee({ type: RefereeType.Center, name: 'Jim' }));
  ...
  test('when game has one center referee then uses pay scale for referee type and age', () => {
    const game = createGame(10, ModelFactory.createReferee({ type: RefereeType.Center, name: 'Jim' }));
    ...
    const checks = RefereeChecksGenerator.generateFromGame(game, [youngerPayScale, correctPayScale, olderPayScale]);
  ...
  test('when game has one assistant referee then uses pay scale for referee type and age group', () => {
    const game = createGame(10, ModelFactory.createReferee({ type: RefereeType.Assistant, name: 'Jim' }));
    ...
    const checks = RefereeChecksGenerator.generateFromGame(game, [otherRefereeType, correctPayScale, olderPayScale]);
  ...
  test('when game age group is between pay scales then uses pay scale for older age group', () => {
    const game = createGame(11, ModelFactory.createReferee({ type: RefereeType.Center, name: 'Jim' }));
    const youngerPayScale = createPayScale(10, 20);
    const correctPayScale = createPayScale(12, 30);

    const checks = RefereeChecksGenerator.generateFromGame(game, [youngerPayScale, correctPayScale]);
  ...
  test('when no matching pay scale is found then uses -1 for amount', () => {
    const game = createGame(12, ModelFactory.createReferee({ type: RefereeType.Center }));
  ...
  test('when game has multiple referees then returns check for each referee', () => {
    const game = createGame(12,
      ...
      ModelFactory.createReferee({ type: RefereeType.Assistant, name: 'wyatt' }),
      ModelFactory.createReferee({ type: RefereeType.Assistant, name: 'sophia' })
    );

    const centerScale = createPayScale(12, 12, RefereeType.Center);
    const assistantScale = createPayScale(12, 6, RefereeType.Assistant);

    const checks = RefereeChecksGenerator.generateFromGame(game, [centerScale, assistantScale]);

    expect(checks).toHaveLength(3);
    expect(checks).toContainEqual(expect.objectContaining({ amount: 12, name: 'stacey' }));
    expect(checks).toContainEqual(expect.objectContaining({ amount: 6, name: 'sophia' }));
    expect(checks).toContainEqual(expect.objectContaining({ amount: 6, name: 'wyatt' }));
  });
  ...
  test('when generating for multiple games then returns referees from all games', () => {
    ...
    const checks = RefereeChecksGenerator.generateFromGames(games, [u10Center, u10Assistant, u8Center]);
  ...
  function createPayScale(age: number, amount: number, refereeType: RefereeType = RefereeType.Center) {
