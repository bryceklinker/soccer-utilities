...
export const GameRow: FunctionComponent<GameRowProps> = ({game}) => {
  const referees = game.referees.map((ref, i) => <Typography key={i} variant={'body1'}>{ref.type}: {ref.name}</Typography>);
  return (
    <TableRow aria-label={'game'}>
      ...
      <TableCell aria-label={'age group'}>U{game.ageGroup.age} {game.ageGroup.gender}</TableCell>
