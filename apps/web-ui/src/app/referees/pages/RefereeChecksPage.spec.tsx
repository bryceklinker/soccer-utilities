...
describe('RefereeChecksPage', () => {
  ...
  test('when loading referee checks then shows loading', () => {
    const store = createTestingStoreFromActions(RefereesActions.loadChecks.request());
    renderWithProviders(<RefereeChecksPage />, { store });
  ...
  test('when referee checks already loaded then shows referee checks', () => {
    const checks = ModelFactory.createListResult(ModelFactory.createClientRefereeCheckModel, 3);
  ...
  test('when referee check copied then notifies referee check written', () => {
    ...
    const store = createTestingStoreFromActions(RefereesActions.loadChecks.success(List.fromItems(check)));
    renderWithProviders(<RefereeChecksPage />, { store });

    userEvent.click(screen.getByRole('button', { name: 'copy check' }));

    expect(store.getActions()).toContainEqual(RefereesActions.checkWritten(check));
  });

  test('when check is written then hides check', () => {
    const check = ModelFactory.createClientRefereeCheckModel({hasBeenWritten: true});
    const store = createTestingStoreFromActions(RefereesActions.loadChecks.success(List.fromItems(check)));
    renderWithProviders(<RefereeChecksPage />, { store });
