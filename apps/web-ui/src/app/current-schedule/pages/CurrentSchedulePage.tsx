...
export const CurrentSchedulePage: FunctionComponent = () => {
  ...
  const isLoadingSchedule = useRootSelector(
    selectIsLoading(CurrentScheduleActions.load.request)
  );
  const hasLoadingErrors = useRootSelector(
    selectHasErrors(CurrentScheduleActions.load.request)
  );
  ...
  return (
    <>
      {!hasLoadingErrors ? (
        <GameSchedule
          schedule={schedule}
          onRefresh={handleLoadSchedule}
          onUpload={handleUploadSchedule}
        />
      ) : null}
      {hasLoadingErrors ? (
        <CurrentScheduleLoadError
          onRetry={handleLoadSchedule}
          onUpload={handleUploadSchedule}
        />
      ) : null}
      {isUploadOpen ? (
